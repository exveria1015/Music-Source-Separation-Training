...
base_args = {
    ...
    'input_folder': '',
    'metrics': ['log_wmse', 'l1_freq', 'si_sdr', 'sdr', 'aura_stft', 'aura_mrstft', 'bleedless', 'fullness'],
    'max_folders': 2
}
...
def parse_args(dict_args):
    ...
    parser.add_argument('--device_ids', nargs='+', type=int, help='Device IDs for training/inference')
    ...
    parser.add_argument('--max_folders', type=int, help='Maximum number of folders to process')
    ...
    parser.add_argument("--loss", nargs='+', help="List of training losses")
    parser.add_argument("--wandb_key", type=str, default='', help='wandb API Key')
    parser.add_argument("--pre_valid", action='store_true', help='Run validation before training')
    parser.add_argument("--metric_for_scheduler", default="sdr",
                        choices=['sdr', 'l1_freq', 'si_sdr', 'log_wmse', 'neg_log_wmse', 'aura_stft', 'aura_mrstft', 'bleedless',
    ...
    parser.add_argument("--train_lora_peft", action='store_true', help="Train with LoRA from peft")
    parser.add_argument("--train_lora_loralib", action='store_true', help="Train with LoRA from loralib")
    parser.add_argument("--lora_checkpoint_peft", type=str, default='', help="Initial checkpoint to PEFT LoRA weights")
    parser.add_argument("--lora_checkpoint_loralib", type=str, default='', help="Initial checkpoint to loralib LoRA weights")
...
def _select_args(settings, keys):
    return {key: settings[key] for key in keys if key in settings and settings[key] is not None}


def _apply_legacy_options(settings):
    legacy_losses = []
    if settings.get('use_multistft_loss'):
        legacy_losses.append('multistft_loss')
    if settings.get('use_mse_loss'):
        legacy_losses.append('mse_loss')
    if settings.get('use_l1_loss'):
        legacy_losses.append('l1_loss')
    if legacy_losses and not settings.get('loss'):
        settings['loss'] = legacy_losses

    if settings.get('train_lora'):
        settings['train_lora_loralib'] = True
    if settings.get('lora_checkpoint') and not settings.get('lora_checkpoint_loralib'):
        settings['lora_checkpoint_loralib'] = settings['lora_checkpoint']
...
def test_settings(dict_args, test_type):
    ...
    if test_type == 'user':
        ...
        # Replace config
        settings['config_path'] = redact_config({'orig_config': settings['config_path'],
                                                  'model_type': settings['model_type'],
                                                  'new_config': ''})

        # Trim train
        trim_args_train = {'input_directory': settings['data_path'],
                           'max_folders': settings['max_folders']}
        settings['data_path'] = trim_directory(trim_args_train)
        # Trim valid
        trim_args_valid = {'input_directory': settings['valid_path'],
                           'max_folders': settings['max_folders']}
        settings['valid_path'] = trim_directory(trim_args_valid)
    # Valid to inference
    valid_path = _first_path(settings['valid_path'])
    if not settings['input_folder']:
        tests_dir = os.path.join(os.path.dirname(valid_path), 'for_inference')
        settings['input_folder'] = tests_dir
    val_to_inf_args = {'valid_path': valid_path,
                       'inference_dir': settings['input_folder'],
                       'max_mixtures': 1}
    copying_files(val_to_inf_args)

    if settings['check_valid']:
        valid_args = _select_args(settings, ['model_type', 'config_path', 'start_check_point',
                                            'store_dir', 'device_ids', 'num_workers', 'pin_memory', 'extension',
                                            'use_tta', 'metrics', 'lora_checkpoint_peft',
                                            'lora_checkpoint_loralib', 'draw_spectro'])
        valid_args['valid_path'] = _as_list(settings['valid_path'])
        print('Start validation.')
        check_validation(valid_args)
        print(f'Validation ended. See results in {settings["store_dir"]}')

    if settings['check_inference']:
        inference_args = _select_args(settings, ['model_type', 'config_path', 'start_check_point', 'input_folder',
                                                'store_dir', 'device_ids', 'extract_instrumental',
                                                'disable_detailed_pbar', 'force_cpu', 'flac_file', 'pcm_type',
                                                'use_tta', 'lora_checkpoint_peft', 'lora_checkpoint_loralib',
                                                'draw_spectro'])

        print('Start inference.')
        proc_folder(inference_args)
        print(f'Inference ended. See results in {settings["store_dir"]}')

    if settings['check_train']:
        train_args = _select_args(settings, ['model_type', 'config_path', 'start_check_point', 'results_path',
                                            'data_path', 'dataset_type', 'valid_path', 'num_workers', 'pin_memory',
                                            'seed', 'device_ids', 'loss', 'wandb_key', 'pre_valid', 'metrics',
                                            'metric_for_scheduler', 'train_lora_peft', 'train_lora_loralib',
                                            'lora_checkpoint_peft', 'lora_checkpoint_loralib'])
